...
const getTemplate = (templatePath) => {
	...
	if (fs.existsSync(templateAbsPath) && !fs.lstatSync(templateAbsPath).isDirectory()) {
...
module.exports = function (webpackEnv) {
				...
				},
				{
					...
					options: {
						additionalData: (content) => {
							let newContent = `@import "${path.resolve(paths.appSrc, 'resources/styles/settings.scss')}"; `;
							const settingsPathExtensions = path.resolve(paths.appExtensions, 'resources/styles/settings.scss');
	...
	return {
		...
		plugins: [
			...
			new CopyPlugin({
				patterns: [{ from: getTemplate('pages/under-construction.html') }]
			}),
			...
			new webpack.NormalModuleReplacementPlugin(
				...
				async (result) => {
					...
					['', '.html', ...paths.moduleFileExtensions.map((ext) => `.${ext}`)]
						.filter((ext) => useTypeScript || !ext.includes('ts'))
						.forEach((ext) => {
							if (fs.existsSync(`${originalPath}${ext}`) && !fs.lstatSync(`${originalPath}${ext}`).isDirectory()) {
								originalExt = ext;
							}
						});
					...
					if (fs.existsSync(`${newPath}${originalExt}`) && !fs.lstatSync(`${newPath}${originalExt}`).isDirectory()) {
						console.log(
