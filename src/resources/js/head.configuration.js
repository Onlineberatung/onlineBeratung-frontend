...
(function (biotope, allowPredefinedData) {
	...
	biotope.configuration.set = function (key, value) {
		...
		if (tempKeys.length === 2) {
			if (
				typeof biotope.configuration.data[tempKeys[0]] === 'undefined'
			) {
	...
	biotope.configuration.get = function (key) {
		...
		if (tempKeys.length === 2) {
			if (
				typeof biotope.configuration.data[tempKeys[0]] ===
					'undefined' ||
				typeof biotope.configuration.data[tempKeys[0]][tempKeys[1]] ===
					'undefined'
			) {
				return null;
			}
