...
import { useLocaleData } from './LocaleProvider';
import getLocationVariables from '../../utils/getLocationVariables';
import decodeHTML from '../../utils/decodeHTML';
import { useAppConfig } from '../../hooks/useAppConfig';

export const TenantContext = createContext<TenantDataInterface>(null);
...
const adjustHSLColor = ({
	...
	adjust: number;
}): string => `hsl(${color.h}, ${color.s}%, ${adjust}%)`;
...
export function TenantProvider(props) {
	...
	const [loading, setLoading] = useState(settings.useTenantService);
	...
	const onTenantServiceResponse = useCallback(
		(tenant: TenantDataInterface) => {
			if (!subdomain && cypressTenantEnabled !== '1') {
				setTenant({ settings } as any);
				setTenantSettings(settings as any);
			} else {
				...
				setTenant(decodedTenant);
				setTenantSettings(decodedTenant.settings);
			}
		},
		[settings, subdomain, cypressTenantEnabled]
	);
